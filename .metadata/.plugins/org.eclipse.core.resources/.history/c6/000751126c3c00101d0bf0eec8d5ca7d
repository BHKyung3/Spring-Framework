...
public class MemberTests {
	...
	public void testInterMember() { // testInterMember()는 회원 데이터 삽입 테스트용 메서드
		...
		for(int i=0; i<100; i++) { // i가 0~99까지 반복하며 100명의 데이터 생성
			...
			try {
				conn = ds.getConnection(); // ds.getConnection(): DB 커넥션 열기
				pstmt = conn.prepareStatement(sql); // conn.prepareStatement(sql): SQL 준비
				// 두번째 ? 자리에 들어갈 값 -> 비밀번호 
				...
				pstmt.executeUpdate(); // 준비된 겂을 DB에 실제로 삽입
	...
	public void validate() {
		if(pwencoder.matches("pw1", "$2a$10$TTb4RyGfW4CljpeC9xAGA.GWdx4Nodj7P2tqwp9yQx/gvTlLHFCGG")) // 암호화 된 비밀번호
			log.info("비밀번호 일치");
		else
			log.info("비밀번호 불일치");
	}
